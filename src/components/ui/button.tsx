	...
	"inline-flex items-center justify-center gap-2 whitespace-nowrap rounded-md text-sm font-medium transition-colors disabled:pointer-events-none disabled:opacity-50 [&_svg]:pointer-events-none [&_svg:not([class*='size-'])]:size-4 shrink-0 [&_svg]:shrink-0 outline-none focus-visible:border-ring focus-visible:ring-ring/50 focus-visible:ring-[3px] aria-invalid:ring-destructive/20 dark:aria-invalid:ring-destructive/40 aria-invalid:border-destructive",
	{
		variants: {
			variant: {
				...
				outline:
					"border border-border bg-background hover:bg-accent hover:text-accent-foreground dark:bg-muted dark:border-muted-foreground/30 dark:hover:bg-muted/80 dark:hover:border-muted-foreground/50",
				secondary:
			...
			size: {
				...
				touch: "h-11 px-5 py-2.5 text-base has-[>svg]:px-4",
				"touch-lg": "h-12 px-6 py-3 text-base has-[>svg]:px-5",
				icon: "size-9",
				"icon-sm": "size-8",
				"icon-lg": "size-10",
				// Touch-optimized icon (44px)
				"icon-touch": "size-11",
			},
		},
		defaultVariants: {
...
function Button({
	...
	// Note: disabled prop is not passed to Slot as it doesn't accept it
	if (asChild) {
		return (
			<Slot
				data-slot="button"
